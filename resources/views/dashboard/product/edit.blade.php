...
<div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14 bg-black dark:bg-gray-900">
    <div class="flex items-center justify-between flex-column md:flex-row flex-wrap space-y-4  p-4 bg-black dark:bg-gray-900">
        ...
        <form class="max-w-md mx-auto bg-gray-9" action="/dashboard/product/{{ $product->id }}/update" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <div class="relative z-0 w-full mb-5 group">
                  <input type="file"  name="gambar" id="gambar" value="{{ $product->gambar }}" class="block py-2.5 px-0 w-full text-sm text-gray-50 bg-transparent border-0 border-b-2 appearance-none border-gray-600 focus:border-blue-500 focus:outline-none focus:ring-0 peer" placeholder=" " required />
                  ...
                  @if ($product->gambar)
                  <img src="{{ asset('storage/' . $product->gambar) }}" class="rounded-2xl w-32 h-32 mt-10">  
                @else
